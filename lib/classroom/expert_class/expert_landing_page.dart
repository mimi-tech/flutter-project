...
import 'package:sparks/classroom/expert_class/expert_key.dart';
...
class _ExpertLandingPageState extends State<ExpertLandingPage>
  ...
  Future<void> verifyUser() async {
    ...
    if ((myKey == null) || (myKey == '')) {
      ...
      setState(() {
        progress = true;
        ExpertConstants.expertCompany.clear();
      });

      try {
        //check if the company key is in the database
        ...
        await FirebaseFirestore.instance
          ...
          value.docs.forEach((result) {
            setState(() {
              key = result.data()['ky'];
              uid = result.data()['uid'];
              ExpertConstants.expertCompany.add(result.data());
            });
          });
        });
        if (key == myKey) {
          ...
          Navigator.of(context).pushReplacement(
              MaterialPageRoute(builder: (context) => ExpertKey()));
        } else {
          setState(() {
            progress = false;
          });
