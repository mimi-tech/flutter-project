...
class UpdatedCourses extends StatefulWidget {
  @override
  _UpdatedCoursesState createState() => _UpdatedCoursesState();
}

class _UpdatedCoursesState extends State<UpdatedCourses> {
  var itemsData = [];
  var _documents = [];

  static var time = DateFormat("hh:mm:a").format(now);

  static var format = DateFormat("hh:mm:a");
  static var presentTime = format.parse(time);
...
/*get the document of courses modified*/

    CourseAdminConstants.updatedCount.clear();
    ...
    try {
      FirebaseFirestore.instance
        ...
        value.docs.forEach((result) {
          //get the courses modified

          FirebaseFirestore.instance
              .collectionGroup('courses')
              .where('id', isEqualTo: result.id)
              .where('se', isEqualTo: true)
            ...
            value.docs.forEach((result) {
              setState(() {
                ...
                CourseAdminConstants.updatedCount.add(result.data());
              });
            });
          });
        });
  ...
  Future<void> verifyThisCourse(DocumentSnapshot document, int index) async {
    ...
    try {
      ...
      documentReference.set({
        'Lc': FieldValue.arrayUnion(lc),
        'targ': FieldValue.arrayUnion(target),
        'obj': FieldValue.arrayUnion(objective),
        'req': FieldValue.arrayUnion(requirement),
        'inc': FieldValue.arrayUnion(include),
        'lang': FieldValue.arrayUnion(language),
        'level': FieldValue.arrayUnion(level),
        'cate': FieldValue.arrayUnion(category),
        'lectures': FieldValue.arrayUnion(lectures),
      ...
      /* push the inspector data to database
*/
...
/*show dialog that ask the user if course has been seen enough*/
  ...
  Future<void> sendTheEmail(int index) async {
    ...
    /*send the sender details to database*/
    setState(() {
      _publishModal = true;
    });
    try {
