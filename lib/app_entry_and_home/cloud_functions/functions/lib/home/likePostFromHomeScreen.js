...
exports.likePostFromHomeScreen = functions.firestore
.document("posts/{userid}/myPost/{postid}/homePostLikes/{documentId}")
.onWrite(async (change, context) => {
    ...
    try {
        ...
        if((previousData === undefined) && (nextData !== undefined)) {
            ...
            //Get the uid of the user who liked the post.
            const getUidWhoLikedPost = nextData.id;

            //Get the fullname of the user who liked the post.
            const fullnameDocument = await admin.firestore().collection('users').doc(getUidWhoLikedPost).collection('Personal').doc('personalInfo').get();
            ...
            //Check if the user uid who liked the post matches with the author's uid who made the post
            ...
            //Check if the user uid who disliked the post matches with the author's uid who made the post
