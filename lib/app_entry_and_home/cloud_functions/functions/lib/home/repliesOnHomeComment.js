...
exports.repliesOnHomeComment = functions.firestore
.document("posts/{userid}/myPost/{postid}/homeComments/{documentId}/commentReplies/{comRpy}")
.onWrite(async (change, context) => {
    ...
    try {
        ...
        if((previousData === undefined) && (nextData !== undefined)) {
            ...
            //Get the fullname of the user who commented on this comment.
            ...
            await admin.firestore().runTransaction(async (transaction) => {
                ...
                documentToUpdate.forEach((eachDocument) => {
                    ...
                    const newNumberOfComments = eachDocument.data().nOfRpy + 1;

                    //Update the number of comments in this post to the current value.
                    transaction.update(eachDocument.ref, {
                        nOfRpy: newNumberOfComments
                    });
               });
            });

            //Check if the user uid who replied this comment does not match with the author's uid who made the comment
