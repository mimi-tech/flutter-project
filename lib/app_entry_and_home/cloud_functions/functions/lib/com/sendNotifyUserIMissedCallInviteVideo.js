...
exports.sendNotifyUserIMissedCallInviteVideo = functions.firestore
.document("calls/{friendId}/date/{dateId}/iMissedVideoCallInvite/{myId}")
.onWrite((change,context)=> {
  ...
  .collection("iMissedVideoCallInvite").doc(myId)
  .get().then((queryResult)=>{
    ...
    const payload={
      ...
      data:{
        ...
        chn:chn,
        ts:ts,
        ca:"TARGET_IMISSEDCALLINVITEVIDEO",
        click_action:"FLUTTER_NOTIFICATION_CLICK"
      }
    };
      
    //console.log(" | usernm: " + my_un + " missed an video call invite from: " + inviter_un + ".");

    return admin.messaging().sendToTopic(friendId, payload);
  
  }).then((response) => {

    console.log("Successfully sent missed video call invite notification to topic id: ", friendId, response);
    
    return null;
    
  }).catch((error)  => {

    console.error("Failure sending missed video call invite notification to topic id: ", friendId, error);
