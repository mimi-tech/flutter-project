...
class DailyTeachersAnalysis extends StatefulWidget {
  DailyTeachersAnalysis({required this.doc});
  final DocumentSnapshot doc;
  @override
  _DailyTeachersAnalysisState createState() => _DailyTeachersAnalysisState();
}

class _DailyTeachersAnalysisState extends State<DailyTeachersAnalysis> {
  ...
  Widget build(BuildContext context) {
    ...
    itemBuilder: (context, int index) {
      return ACardConstruct(
        text6: '${DateFormat('EE, d MMM, yyyy').format(DateTime.parse(workingDocuments[index]['ts'].toString()))}',
  ...
  void getDailyAnalysis() {
    try{
      FirebaseFirestore.instance.collectionGroup('dailyTeachersAnalysis')
