...
import 'package:sparks/schoolClassroom/schoolPost/postLikes.dart';
import 'package:sparks/schoolClassroom/schoolPost/replyPostComments.dart';

import 'package:timeago/timeago.dart' as timeago;

class SchoolPostComments extends StatefulWidget {
  SchoolPostComments({required this.doc});
  final DocumentSnapshot doc;
  @override
  _SchoolPostCommentsState createState() => _SchoolPostCommentsState();
}

class _SchoolPostCommentsState extends State<SchoolPostComments> {
  bool progress = false;

  List<Map<String, dynamic>> workingDocuments = [];
  var _documents = [];
        ...
        children: [
          ListView.builder(
              ...
              itemBuilder: (context, int index) {
                return workingDocuments.length == 0 && progress == false
                                      ...
                                      children: [
                                        IconButton(
                                            icon: Icon(Icons.thumb_up,
                                                color: kbtnsecond),
                                            onPressed: () {
                                              _likeComment(workingDocuments as List<DocumentSnapshot<Object>?>, index);
                                            }),
                                        Text(
                                    ...
                                    Row(
                                      children: [
                                        IconButton(
                                            ...
                                            onPressed: () {
                                              _replyComment(
                                                  workingDocuments as List<
                                                      DocumentSnapshot<
                                                          Object>?>,
                                                  index);
                                            }),
                                        Text(
                                          '${workingDocuments[index]['re']}',
                                          style: GoogleFonts.rajdhani(
                                            textStyle: TextStyle(
                                              fontWeight:
                                              FontWeight.bold,
  ...
  Future<void> _likeComment(List<DocumentSnapshot?> workingDocuments, int index) async {
    //check if comment have been liked by this student

    final QuerySnapshot result = await FirebaseFirestore.instance
        .collection('schoolLikedComment')
    ...
    if (documents.length == 0) {
      ...
      FirebaseFirestore.instance.collection('schoolPostLikes').doc();
      docRef.set({
        'id': docRef.id,
  ...
  void _replyComment(List<DocumentSnapshot?> workingDocuments, int index) {
    Navigator.push(
        ...
        PageTransition(
            ...
            child: SchoolPostReplyComments(doc: workingDocuments[index])));
  }

  void _viewLikes(List<Map<String, dynamic>?> workingDocuments, int index) {
//viewLikes
...
class PostCommentStream extends StatefulWidget {
  PostCommentStream({required this.doc});
  final DocumentSnapshot doc;
  @override
  _PostCommentStreamState createState() => _PostCommentStreamState();
}

class _PostCommentStreamState extends State<PostCommentStream> {
    ...
    return SafeArea(
      child: Scaffold(
        ...
        body: Column(
          ...
          children: <Widget>[
            SchoolPostComments(doc: widget.doc),
            Container(
  ...
  _sendComment() {
    ...
    try {
      ...
      docRef.set({
        ...
        //'lv':widget.doc['topic'] == null?widget.doc['title']:widget.doc['topic'],
      ...
      //increase the number of comment for the post

      FirebaseFirestore.instance
          .collection('schoolPost')
          .doc(widget.doc['schId'])
