...
class ViewClassNote extends StatefulWidget {
  ViewClassNote({required this.note, required this.doc});
  final DocumentSnapshot doc;
  final dynamic note;
  @override
  _ViewClassNoteState createState() => _ViewClassNoteState();
}

class _ViewClassNoteState extends State<ViewClassNote> {
  int? progress = 0;
  ...
  Future<void> updateNoteDownload() async {
    try {
      ...
      if (documents.length == 0) {
        ...
        FirebaseFirestore.instance
            .collection('studentsNoteLessons')
            .doc(widget.doc['schId'])
            .collection('noteLessons')
            .add({
          'id': widget.doc['id'],
