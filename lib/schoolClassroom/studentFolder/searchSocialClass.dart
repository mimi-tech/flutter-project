...
class SearchSocialClass extends StatefulWidget {
  @override
  _SearchSocialClassState createState() => _SearchSocialClassState();
}

class _SearchSocialClassState extends State<SearchSocialClass> {
  ...
  initiateSearch(value) {
    ...
    if (queryResultSet.length == 0 && value.length == 1) {
      SearchService()
          .searchByStudentSocialClass(value)
    ...
    return SafeArea(
        child: Scaffold(
            appBar: AppBar(
              ...
              title: Text(
                'Search for an extra curricular lecture',
                                ...
                                children: [
                                  Row(
                                    children: [
                                      ...
                                      SocialShowLiveText(
                                        likeClick: () {
                                          _likeLesson(tempSearchStore[index]);
                                        },
                                        title: data['title'],
                                        desc: data['desc'],
...
//give your teacher a feedback concerning this lesson
  ...
  Future<void> _likeLesson(DocumentSnapshot doc) async {
    ...
    try {
      ...
      final QuerySnapshot result = await FirebaseFirestore.instance
          .collectionGroup('likedSocialClass')
      ...
      if (documents.length >= 1) {
        ...
        FirebaseFirestore.instance
            .collection('socialClass')
            .doc(data['schId'])
            .collection('schoolSocials')
            .doc(data['id'])
            .get()
            .then((resultLike) {
        ...
        FirebaseFirestore.instance
            .collection('studentsLikedSocial')
            .doc(data['schId'])
            .collection('likedSocialClass')
            .add({
          'id': data['id'],
  ...
  Future<void> updateViews(DocumentSnapshot doc) async {
    ...
    try {
      //check if student have liked this lesson
      final QuerySnapshot result = await FirebaseFirestore.instance
          .collectionGroup('viewedSocialClass')
          .where('id', isEqualTo: data['id'])
          .where('pin', isEqualTo: SchClassConstant.schDoc['pin'])
          .get();
      ...
      if (documents.length == 0) {
        ...
        FirebaseFirestore.instance
            .collection('studentsViewedSocial')
            .doc(data['schId'])
            .collection('viewedSocialClass')
            .add({
          'id': data['id'],
          'pin': SchClassConstant.schDoc['pin'],
          'fn': SchClassConstant.schDoc['fn'],
  ...
  Future<void> updateDownloads(DocumentSnapshot doc) async {
    ...
    try {
      ...
      final QuerySnapshot result = await FirebaseFirestore.instance
          .collectionGroup('downloadedSocialClass')
          .where('id', isEqualTo: data['id'])
          .where('pin', isEqualTo: SchClassConstant.schDoc['pin'])
          .get();
      ...
      if (documents.length == 0) {
        ...
        //push lesson to view lessons
        FirebaseFirestore.instance
            .collection('studentsDownloadedSocial')
            .doc(data['schId'])
            .collection('downloadedSocialClass')
            .add({
          'id': data['id'],
          'pin': SchClassConstant.schDoc['pin'],
          'fn': SchClassConstant.schDoc['fn'],
