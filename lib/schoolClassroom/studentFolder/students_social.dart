...
import 'package:sparks/schoolClassroom/social_showText.dart';
import 'package:sparks/schoolClassroom/studentFolder/searchLessons.dart';
import 'package:sparks/schoolClassroom/studentFolder/searchSocialClass.dart';
import 'package:sparks/schoolClassroom/studentFolder/social_feedback.dart';
...
class StudentSocial extends StatefulWidget {
  @override
  _StudentSocialState createState() => _StudentSocialState();
}

class _StudentSocialState extends State<StudentSocial> {
  int progress = 0;
    ...
    return Scaffold(
     // bottomNavigationBar:EClassBottomBar(),
      ...
      body:CustomScrollView(slivers: <Widget>[
        ...
        NewsBoardAppBar(
          extraLessonsBgColor: klistnmber,
          extraLessonsColor: kWhitecolor,
        ...
        EClassSliverAppBarSearchSecond(
          searchTap: (){
            Navigator.push(context, PageTransition(type: PageTransitionType.fade, child: SearchSocialClass()));
...
//give your teacher a feedback concerning this lesson
    showModalBottomSheet(
        ...
        builder: (context) {
          return StudentSocialFeedback(doc:doc);
        });
  ...
  Future<void> _likeLesson(DocumentSnapshot doc) async {
    ...
    try{
      ...
      final QuerySnapshot result = await FirebaseFirestore.instance.collectionGroup('likedSocialClass')
      ...
      if (documents.length >= 1) {
        ...
        FirebaseFirestore.instance.collection('socialClass').doc(
            data['schId']).collection('schoolSocials').doc(
            data['id']).get()
            .then((resultLike) {
        ...
        FirebaseFirestore.instance.collection('studentsLikedSocial').doc(data['schId']).collection('likedSocialClass').add({
          'id':data['id'],
  ...
  Future<void> updateViews(DocumentSnapshot doc) async {
    ...
    try{
      //check if student have liked this lesson
      final QuerySnapshot result = await FirebaseFirestore.instance.collectionGroup('viewedSocialClass')
          .where('id', isEqualTo: data['id'])
          .where('pin', isEqualTo: SchClassConstant.schDoc['pin'])
      ...
      if (documents.length == 0) {
        ...
        FirebaseFirestore.instance.collection('studentsViewedSocial').doc(data['schId']).collection('viewedSocialClass').add({
          'id':data['id'],
          'pin':SchClassConstant.schDoc['pin'],
          'fn':SchClassConstant.schDoc['fn'],
          'ln':SchClassConstant.schDoc['ln'],
  ...
  Future<void> updateDownloads(DocumentSnapshot doc) async {
    ...
    try{
      ...
      final QuerySnapshot result = await FirebaseFirestore.instance.collectionGroup('downloadedSocialClass')
          .where('id', isEqualTo: data['id'])
          .where('pin', isEqualTo: SchClassConstant.schDoc['pin'])
      ...
      if (documents.length == 0) {
        ...
        FirebaseFirestore.instance.collection('socialClass').doc(data['schId']).collection('schoolSocials').doc(
            data['id']).get()
        ...
        //push lesson to view lessons
        FirebaseFirestore.instance.collection('studentsDownloadedSocial').doc(data['schId']).collection('downloadedSocialClass').add({
          'id':data['id'],
          'pin':SchClassConstant.schDoc['pin'],
          'fn':SchClassConstant.schDoc['fn'],
          'ln':SchClassConstant.schDoc['ln'],
