...
import 'package:sparks/jobs/subScreens/professionals/create/screenTen.dart';
import 'package:sparks/school_reg/screens/school_constance.dart';
import 'package:sparks/app_entry_and_home/colors/colour.dart';
import 'package:sparks/jobs/components/generalComponent.dart';
import 'package:sparks/app_entry_and_home/static_variables/static_variables.dart';

class CreateProfessionalScreenNine extends StatefulWidget {
  final offsetBool;
  final double? widthSlide;
  final Widget? example;
  CreateProfessionalScreenNine({
    Key? key,
    this.offsetBool,
    this.widthSlide,
    this.example,
  ...
  @override
  _CreateProfessionalScreenNineState createState() =>
      _CreateProfessionalScreenNineState();
}

class _CreateProfessionalScreenNineState
    extends State<CreateProfessionalScreenNine> {
  ...
  List<Widget> showServiceInput = [];
  List<Widget> serviceDisplay = [];
  int count = 0;

  var listServices = [];

  void displayAll() {
    if (ProfessionalStorage.hasService == "yes") {
      listServices = ProfessionalStorage.services;
      showServiceForm();
      displayService();
    }
  }

  String? answerGroupValue = ProfessionalStorage.hasService;
  void changeAnswerTypeState(value) {
    if (value == "yes") {
      setState(() {
        answerGroupValue = "yes";
        ProfessionalStorage.hasService = "yes";
        showServiceForm();
      });
    } else if (value == "no") {
      setState(() {
        answerGroupValue = "no";
        ProfessionalStorage.hasService = "no";
        showServiceInput = [];
        serviceDisplay = [];
        count = 0;
  ...
  void showServiceForm() {
    setState(() {
      showServiceInput.add(
        Column(children: [
          Padding(
            padding: const EdgeInsets.fromLTRB(0.0, 10.0, 0.0, 0.0),
            child: HintText(
              hintText: "Awesome! Tell us about your services",
            ),
          ),
          Padding(
...
//TODO: create the object with the input text values
      ...
      ProfessionalStorage.services.add(service);
...
//TODO: display the data in the list of maps
          ...
          child: Column(
            children: <Widget>[
              ...
              Padding(
                padding: const EdgeInsets.fromLTRB(0.0, 10.0, 0.0, 0.0),
                child: HintText(
                  hintText: "Do you offer any professional service?",
                ),
              ),
              ...
              Container(
                  ...
                  child: Column(
                    children: serviceDisplay,
                  )),
              Column(
                children: showServiceInput,
              ),
  ...
  void goToNext() {
    print(ProfessionalStorage.hasService);
    print(ProfessionalStorage.services);

    Navigator.of(context).push(
        MaterialPageRoute(builder: (context) => CreateProfessionalScreenTen()));
  }
}
